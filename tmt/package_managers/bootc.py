...
class BootcEngine(PackageManagerEngine):
    ...
    def _get_current_bootc_image(self) -> str:
        ...
        command += Command('status', '--format', 'humanreadable')
        ...
        match = re.match(r'Booted image: (\S+)\\n', str(output.stdout))
        if not match:
            raise tmt.utils.PrepareError(
                f"Failed to parse bootc status: invalid output: {output.stdout}"
            )

        return match.group(1)

    def _get_base_containerfile_directives(self) -> list[str]:
        return [f'FROM {self._get_current_bootc_image()}']
...
class Bootc(PackageManager[BootcEngine]):
    ...
    def build_container(self) -> None:
        ...
        with self.guest.mkdtemp() as containerfile_dir:
            ...
            try:
                # Check if image exists and store it from bootc of don't
                # self.guest.execute(
                #     ShellScript(
                #         f'podman images {base_image} | grep {base_image} || bootc image copy-to-storage --target {base_image}'  # noqa: E501
                #     )
                # )
                self.guest.execute(
                ...
                # Re-intitialize containerfile directives
